...
### Step 1: Check out the probe_scan branch

```bash
cd ~/klipper
git remote add probe-scan https://github.com/mairas/klipper.git
git fetch probe-scan
git checkout probe-scan/feat/probe-scan-ratos
```

Note: this replaces your Klipper installation with the probe_scan
branch. To revert, check out your original branch (e.g.,
`git checkout master` for upstream or the RatOS branch).

### Step 2: Rebuild and flash MCU firmware

The probe_scan module includes MCU firmware (`src/probe_scan.c`) that
polls the probe GPIO and timestamps state transitions. The firmware
module auto-registers when any GPIO is available; no Kconfig changes are
needed.

```bash
cd ~/klipper
make clean
make menuconfig  # Select your board (same settings as before)
make
```

Flash according to your board's procedure, for example:
```bash
...
The Python module (`klippy/extras/probe_scan.py`) loads automatically
when a `[probe_scan]` section exists. Add the following to
`printer.cfg`:
