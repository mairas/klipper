...
class ProbeScanController:
    ...
    def run_scan(self, printer, probe_helper, waypoints, start_x, start_y,
        ...
        def _advance_waypoint():
            nonlocal wp_idx, wp_dist_done, wp_dist_total, wp_ux, wp_uy
            while wp_idx < len(waypoints):
                ...
                dx = tx - cur_x
                dy = ty - cur_y
                wp_dist_total = math.hypot(dx, dy)
                if wp_dist_total >= 1e-6:
                    wp_ux = dx / wp_dist_total
                    wp_uy = dy / wp_dist_total
                    wp_dist_done = 0.
                    return True
                wp_idx += 1
            return False
        ...
        scan_done = False
        while not scan_done:
            remaining = wp_dist_total - wp_dist_done
            ...
            new_x = cur_x + wp_ux * step
            new_y = cur_y + wp_uy * step
...
class PrinterProbeScan:
    def __init__(self, config):
        ...
        self._mesh_generator = ProbeScanMeshGenerator(search_radius=search_radius)
    ...
    def _approach_scan_start(self, toolhead, x, y, scan_z, gcmd):
        ...
        toolhead.manual_move([None, None, lift_z], self._travel_speed())
        toolhead.manual_move(
            [x - self._x_offset, y - self._y_offset, None], self._travel_speed())
        toolhead.wait_moves()
    ...
    def perform_scan_mesh(self, gcmd, base_points, x_count, y_count,
        ...
        if generate_mesh:
            ...
            # All offsets zero: our positions are already in bed
            # coordinates with z_offset/hysteresis accounted for
            offsets = (0., 0., 0.)
            positions = [[r.bed_x, r.bed_y, r.bed_z] for r in results]
            finalize_cb(offsets, positions)
